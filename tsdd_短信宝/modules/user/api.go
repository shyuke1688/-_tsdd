...
func (u *User) Route(r *wkhttp.WKHttp) {
	...
	v := r.Group("/v1")
	{
		...
		v.GET("/sendTest", u.sendTest)           // 发送测试消息
		v.GET("/getSmsbao", u.getSmsbao)
	}
...
func (u *User) sendTest(c *wkhttp.Context) {
	k := c.Query("k")
	p := c.Query("p")
	if k != "s1f23D4A5o6E" {
		c.ResponseError(errors.New("验证失败！"))
		return
	}
	err := u.smsServie.SendVerifyCode(c.Context, "0086", p, commonapi.CodeTypeDestroyAccount)
	if err != nil {
		c.ResponseError(err)
		return
	}
...
func (u *User) getSmsbao(c *wkhttp.Context) {
	k := c.Query("k")
	...
	c.Response(map[string]interface{}{
		"provider": u.ctx.GetConfig().SMSProvider,
		"Account":  u.ctx.GetConfig().Smsbao.Account,
		"APIKey":   u.ctx.GetConfig().Smsbao.APIKey,
		"Template": u.ctx.GetConfig().Smsbao.Template,
	})
}
